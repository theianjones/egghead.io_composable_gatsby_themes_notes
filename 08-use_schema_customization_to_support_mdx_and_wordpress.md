...
00:30 We need to look at these files to see what fields our themes are using:
...
01:05 We create a `gatsby-theme-blog-data/gatsby-node.js` file. We are going to create a [`graphql` interface](https://graphql.org/learn/schema/#interfaces). We need to us [Gatsby's `@nodeInterface`](https://www.gatsbyjs.org/docs/node-interface/) as well.
...
01:22 We are going to use `gatsby-theme-blog-data` in our `gatsby-theme-product-blog`. This creates a parent-child relationship between the two themes. The `blog-data` theme is the parent while the `product-blog` theme is the child that uses and extends it.

01:44 We will add `gatsby-theme-blog-data` to `packages/gatsby-theme-product-blog/gatsby-config.js` and `packages/gatsby-theme-product-blog/package.json`
...
02:19 We just added an `allBlogPost` type to our graphQL layer. You can view the schema under `localhost:8000/__graphql`. **Note that we don't have any data that is considered a BlogPost, or satisfies this interface, so we don't get any results, but we also don't get any errors if we query it.**

02:37 By declaring allBlogPosts as an interface, our templates can now rely on the fact that allBlogPosts exist. This means we can source our data from anywhere, and as long as it fits the blogpost interface, our templates can handle it.

02:59 To be able to query WordPress posts, we need to create a set of concrete nodes that implement a blogpost interface. Concrete nodes are literal graphql nodes that satisfy an abstract interface. So our WordPress posts can implement the blog interface becoming concrete nodes of that interface.

03:20 Before we create the concrete nodes, we need to create the schema type for these concrete nodes. This tells graphql that the WordPress posts implement the BlogPost interface. We use `createSchemaCustomization` lifecycle method for this.
...
03:40 We also declared to be a `@childOf` a specific type. In this case, we've declared it to be a `@childOf` `"wordpress__Post"`. Though if we ever need to, **we can actually access a blogpost WordPress as a `@childOf` any WordPress post, allowing us to make more intricate queries if we ever need to break outside the blogpost interface.**

04:12 Now we can create concrete nodes from the `blogpostWordPressType`. To create concrete nodes, we'll `require crypto` at the top of the file. Then we'll introduce an onCreate node lifecycle call.
...
04:42 If the type we're operating on in onCreate node isn't a `wordpress\_\_POST`, we don't want to do anything, so we just return.

04:59 In this case, **all the fields exist on the node and none of them need to be resolved**.
...
05:29 If you try to query `allBlogPost`, the query will still return nothing. This is because of Gatsby' cache system. You will need to `rm -rf www/.cache` or `gatsby clean` whenever you update the `onCreateNode` lifecycle method.

06:19 We now want to query `allBlogPost` instead of `allWordPressPost` in our project.
...
06:33 Now we need to change the query inside of our `wordpress-blog-post.js` template.
...
06:54 We need to make theses same changes in `packages/gatsby-theme-product-blog/src/pages/blog.js`.
...
07:11 **Any concrete node you can think of that you can get to implement this interface can be returned here.**
